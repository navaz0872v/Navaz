...
TELEGRAM_BOT_TOKEN = '7569184938:AAEQ8cY2RtXr5dtjqhwNa6yWvlrohQV1MD04'
...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*access kon tera baap lega lode @Mafia_op24! /get_id*", parse_mode='Markdown')
        return
    message = (
        "*WELCOME 🤗 TO MAFIA PAID DDOS *\n\n"
        "*💀Use /attack <ip> <port> <duration>*\n"
        "*💀sever ki chudayi @Mafia_op24 🚀*" 
    )
...
async def attack(update: Update, context: CallbackContext):
    ...
    user_id = update.effective_user.id
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*access kon tera baap lega lode @Mafia_op24!*", parse_mode='Markdown')
        return
    args = context.args
    if len(args) != 3:
    ...
    text=(
        ...
        f"*🔥chudai chalu h feedback bhej dena @Mafia_op24💥*"
    ), parse_mode='Markdown')
